...
const handleGetResidents = async (id) => {
  try {
    ...
    const promesas = res.map((characterUrl) => handleGetCharacterData(characterUrl));
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  searchButton.addEventListener("click", async function () {
    const locationId = locationInput.value ? parseFloat(locationInput.value) : null;
    ...
    if (residentes.length) {
      ...
      characterImages.forEach((img) => {
        img.addEventListener("click", (e) => {
          const characterIndex = e.target.getAttribute("data-character-index");
      ...
      const cardHtml = `
        <div class="residentsIsEmpty">
            <h2>Esta ubicación no tiene residentes</h2>
        </div>
        `;
